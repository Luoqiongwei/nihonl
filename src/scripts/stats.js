...
function render() {
  const s = store.stats();
  const dist = store.levelDistribution();
  const labels = ['未学', '刚认识', '学习中', '较熟', '熟悉', '掌握'];
  const activity = store.recentActivity(14);
  ...
  root.querySelector('#goal-save').addEventListener('click', () => {
    store.setDailyGoal(goalInput.value);
    render();
    import('../lib/helpers.js').then(({ toast }) => toast('每日目标已更新'));
  });
  root.querySelector('#reset-btn').addEventListener('click', () => {
    if (confirm('确定要清空所有学习进度吗？此操作不可撤销。')) {
      store.reset();
      render();
      import('../lib/helpers.js').then(({ toast }) => toast('进度已重置'));
    }
  });
}
